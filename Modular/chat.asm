...
scroll_window_up proc
                              ...
                              mov  dh, 12                       ; Bottom row of the window (10)   i.e Y2
...
scroll_window_up_receiver proc
                              ...
                              mov  cl, 0                        ; Left column of the window (0)   i.e X1
                              mov  dh, 24                       ; Bottom row of the window (23)   i.e Y2
                              mov  dl, 79                       ; Right column of the window (79) i.e X2
                              int  10h                          ; Call BIOS interrupt
...
movcurserSender proc far
    ...
    ; Check if row_sender exceeds 10
                              cmp  row_sender, 13
                              jl   skip_scroll                  ; If within limits, skip scrolling
                              call scroll_window_up             ; Scroll up
                              mov  row_sender, 12               ; Keep cursor on the last allowed row
...
moveCursorReceiver proc far
    ...
    ; Check if row_receiver exceeds 24
                              cmp  row_receiver, 25
                              jl   skip_scroll_recev            ; If within limits, skip scrolling
                              call scroll_window_up_receiver    ; Scroll up
                              mov  row_receiver, 24             ; Keep cursor at the last allowed row
...
chat proc far
    ...
    ; else flag is set to 1 if a key presses
                              ...
                              mov  dl, column_sender
                              mov  dh, row_sender
                              call SetCursorPosition            ; Set cursor to test the positio
    EMPTYlOPP:                
    ...
    flag1:                    
                              ...
                              int  10h                          ; AL contains the character at the current position
                              cmp  al, ' '
                              JE   EMPTYlOPP
                              jmp  SkipEnter
    NotBackSpace:             
    ...
    EnterChar:                
                              ...
                              cmp  row_sender, 13               ; Check if the current row exceeds the top 10 rows
                              jne  skipme                       ; If not, skip scrolling
    ...
    skipme:                   
                              mov  dl, column_sender            ; Set cursor column
                              mov  dh, row_sender               ; Set cursor row
    ...
    EnterCharReceiver:        
                              ...
                              cmp  row_receiver, 25             ; Check if the current row exceeds the top 10 rows
                              jne  skipmeR                      ; If not, skip scrolling

                              call scroll_window_up_receiver    ; Scroll up
                              
                              dec  row_receiver                 ; Adjust row_sender after scrolling (stay at bottom line of sender's area)

    skipmeR:                  
                              mov  dl, column_receiver          ; Set cursor column
                              mov  dh, row_receiver             ; Set cursor row
                              call SetCursorPosition            ; Update cursor position
